describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let [index, prodItem] of prodItems.entries()) {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const finalCount = await cartCount.getProperty('innerText');
    const finalNumber = await finalCount.jsonValue();
    expect(finalNumber).toBe('20');
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const countBefore = await page.$eval('#cart-count', cartCount => cartCount.innerHTML);
    await page.reload();
    const countAfter = await page.$eval('#cart-count', cartCount => cartCount.innerHTML);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const cartStorage = await page.evaluate(() => JSON.parse(localStorage.getItem('cart')));
    expect(cartStorage).toEqual(expected);
  });
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let prodItem of prodItems) {
      ...
      const buttonProp = await button.getProperty('innerHTML');
      const buttonInner = await buttonProp.jsonValue();
      expect(buttonInner).toBe('Add to Cart');
    }
    const cartCount = await page.$eval('#cart-count', cartCount => cartCount.innerHTML);
    expect(cartCount).toBe('0');
  }, 10000);
